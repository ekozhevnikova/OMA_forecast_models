...
class Forecast_Models:
    ...
    def rolling_mean_fixed_periods(self):
        #TODO выбирать количество исторических значений
        """
                ...
                :return:DataFrame с прогнозом на следующий год, включающим прогнозируемый тренд и сезонность.

                """
        # Выделение данных за последние три года
        last_three_years = self.df[self.df.index >= self.df.index[-1] - pd.DateOffset(years=3)]
        # Скользящее среднее за 12 месяцев
        rolling_mean = last_three_years.rolling(window=12).mean()
        detrended = last_three_years - rolling_mean

        # Прогнозируем сезонность
        last_detr = detrended.iloc[-12:].reset_index(drop=True)
        penult_detr = detrended.iloc[-24:-12].reset_index(drop=True)
        ...
        next_year_rolling_mean = [last_value + (i + 1) * average_step for i in range(12)]
        next_year_dates = pd.date_range(start=last_three_years.index[-1] + pd.DateOffset(months=1), periods=12,
                                        freq='MS')
    ...
    def rolling_mean_calendar_years(self):
        #TODO выбирать количество исторических значений
        """
           Функция для выделения тренда и сезонности из временного ряда данных,
           а также прогнозирования тренда и сезонности на следующие два года (24 месяца).

           :param data: DataFrame с временным рядом
           :return: DataFrame с прогнозом и список рассматриваемых исторических значений
           """
        ...
        if last_month < 12:
            ...
            # Выделяем сезонность
            last_detr = detrended[detrended.index.year == end_year - 1].reset_index(drop=True)
            penult_detr = detrended[detrended.index.year == end_year - 2].reset_index(drop=True)
            ...
            penult_detr = detrended[detrended.index.year == end_year - 1].reset_index(drop=True)
            seasonality = (last_detr + penult_detr) / 2
        ...
        next_year_rolling_mean = [last_value + (i + 1) * average_step for i in range(24)]
        next_year_dates = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=24, freq='MS')
        next_year_rolling_mean_df = pd.DataFrame(next_year_rolling_mean, index=next_year_dates, columns=self.df.columns)

        # Прогноз сезонности (повторяем 12 месяцев шаблона)
        seasonality_repeated = np.tile(seasonality.values, (24 // 12 + 1, 1))[:24]

        # Финальный прогноз: сложение тренда и сезонности
        final_forecast = next_year_rolling_mean_df.values + seasonality_repeated
        forecast_df = pd.DataFrame(final_forecast, index=next_year_dates, columns=self.df.columns)

        return forecast_df.head(12), last_n_list
