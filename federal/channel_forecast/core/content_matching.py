...
class SportChannelCleaner:
    ...
    Оптимизированная версия с кэшированием regex для городов
    """
    
    def __init__(self, channel: str, cache_size: int = 1000):
        self.channel = channel
        
        # Кэш для скомпилированных regex городов
        self._city_regex_cache: Dict[Tuple[str, ...], re.Pattern] = {}
        
        # Кэш для результатов remove_cities (LRU)
        self._remove_cities_cache = OrderedDict()
        self._cache_maxsize = cache_size
        
        # Кэш для preparatory_cleaner
        self._prep_cache: Dict[str, str] = {}
        
        # Инициализация конфигураций (ДО компиляции паттернов)
        self._init_configs()
        
        # Компилируем статические паттерны один раз
        self._compile_static_patterns()
        
        # Сортируем стоп-паттерны и стоп-слова заранее
        self._sorted_stop_patterns = {}
        self._sorted_stop_words = {}
        for cat in ['films', 'sport']:
            if cat in self.STOP_PATTERNS:
                self._sorted_stop_patterns[cat] = sorted(
                    self.STOP_PATTERNS[cat], key = len, reverse = True
                )
            if cat in self.STOP_WORDS:
                self._sorted_stop_words[cat] = sorted(
                    self.STOP_WORDS[cat], key = len, reverse = True
                )

    
    def _init_configs(self):
        """
            Инициализация всех конфигурационных списков и словарей
        """
        ...
        self.STOP_PATTERNS = {
            'films': [
                ...
                r'\bтелесериал\b', r'\bюмористический\b', r'\bкороткометражные\b', r'\bфильм\b', 
                r'\bсериал\b', r'\bхуд\.?\s*', r'\bцикл\b', 
                r'\b[aадхмmx][^а-яА-Яa-zA-Z0-9]+ф\b',
            ],
            'sport': [r'\bповтор\b', r'\bс[\s.-]*п[\s.-]*б\b', r'\bсуперигра\b']
        }
        
        # ВАЖНО: SPONSORS должен быть доступен как атрибут класса
        self.SPONSORS = [
            'лига ставок', 'betboom', 'olimpbet', 'g drive',
            'альфа банк', 'winline', 'fonbet', 'фонбет',
            'озон', 'ozon', 'фосагро', 'технониколь', 
        ...
        self.STOP_WORDS = {
            'not_sport': {'спецрепортаж', 'дайджест'},
            'sport': {
                'сезон', 'дайджест', 'место', 'раунд',
                'online', 'on line', 'открытый чемпионат россии', 'кубок гагарина',
                'зимняя серия игр', 'весенняя серия игр', 'летняя серия игр', 'осенняя серия игр'
            },
            'films': {'фильм', 'документальный', 'сериал', 'fonbet'}
        }
    

    def _compile_static_patterns(self):
        """
            Предварительная компиляция всех статических regex паттернов
        """
        self._compiled = {}
        
        # Паттерны для preparatory_cleaner
        age_patterns = [
            r'\(\s*\+?(?:6|12|14|16|18)\+?\s*\)',
            r'(?:^|\s)\+(?:6|12|14|16|18)(?=\s|$)',
            r'(?:^|\s)(?:6|12|14|16|18)\+(?=\s|$)',
        ...
        self._compiled['age'] = re.compile('|'.join(age_patterns), re.IGNORECASE)
        
        # Паттерн для удаления номеров с решёткой
        self._compiled['hash_numbers'] = re.compile(r'[n#№]\s*\d+')
        
        # Паттерн для пунктуации
        self._compiled['punctuation'] = re.compile(r'[^а-яА-Яa-zA-Z0-9\s]')
        
        # Паттерны для удаления номеров частей/серий/сезонов/фильмов
        self._compiled['parts'] = re.compile(
            r'\b(?:(?:\d+|[ivxlcdm]+)\s+(?:часть|части|ч)\b|\b(?:часть|части|ч)\s+(?:\d+(?:\s+\d+)*|[ivxlcdm]+))\b',
            re.IGNORECASE
        )
        self._compiled['series'] = re.compile(
            r'\b(?:(?:\d+|[ivxlcdm]+)(?:\s+(?:\d+|[ivxlcdm]+))*\s+(?:серия|серии|сер)\b|\b(?:серия|серии|сер)\s+(?:\d+|[ivxlcdm]+)(?:\s+(?:\d+|[ivxlcdm]+))*)\b',
            re.IGNORECASE
        )
        self._compiled['seasons'] = re.compile(
            r'\b(?:(?:\d+|[ivxlcdm]+)\s*(?:сезон|сезона|сезонов)\b|\b(?:сезон|сезона|сезонов)\s+(?:\d+|[ivxlcdm]+)(?:\s+(?:\d+|[ivxlcdm]+))*)\b',
            re.IGNORECASE
        )
        self._compiled['films_num'] = re.compile(
            r'\b(?:фильм|фильма|фильмов)\s+(?:\d+|[ivxlcdm]+)(?:[-\s]*[йой])?(?:\s+(?:\d+|[ivxlcdm]+)(?:[-\s]*[йой])?)*\b',
            re.IGNORECASE
        )
        
        # Паттерны для финальной очистки
        self._compiled['four_digits'] = re.compile(r'\d{4,}\s*(?:[гГ]\.?)?(?=\s|$)')
        self._compiled['digit_letters'] = re.compile(r'\d+\s*[-—]?\s*[а-яА-Я]{1,2}(?!\S)')
        self._compiled['spaces'] = re.compile(r'\s+')
        
        # Паттерны для clean_sport_programs
        self._compiled['final_stage'] = re.compile(r'\s*1[\s/\-]*(?:2|4|8|16)[\s/\-]*финала\s*')
        self._compiled['match_number'] = re.compile(r'\s*\d+\s*й\s*матч\s*')
        self._compiled['attempt_number'] = re.compile(r'\d+\s*я\s*попытка', re.IGNORECASE)
        self._compiled['letter_digit'] = re.compile(r'[a-zA-Z]+\d+(?!\S)')
        
        # Комбинированный паттерн для финала/полуфинала и т.д.
        variants = ['полуфинал', 'четвертьфинал', 'финал', 'жен', 'муж', 'премьера']
        self._compiled['final_variants'] = re.compile(
            r'\s*(?:' + '|'.join(variants) + r')\s*', re.IGNORECASE
        )
        
        # Паттерны для clean_not_sport и clean_films
        self._compiled['not_major'] = re.compile(
            r'\b(?:повтор|премьера|часть)\b', re.IGNORECASE
        )
        
        # Паттерн для спонсоров (используем self.SPONSORS)
        sorted_sponsors = sorted(self.SPONSORS, key = len, reverse = True)
        escaped_sponsors = [re.escape(s) for s in sorted_sponsors]
        self._compiled['sponsors'] = re.compile(
            r'\s*(?:' + '|'.join(escaped_sponsors) + r')\s*', re.IGNORECASE
        )

    
    def _get_cache_key(self, cities_list: List[str]) -> Tuple[str, ...]:
        """
            Создаёт ключ для кэша городов
        """
        return tuple(sorted(cities_list))
    
    
    def remove_cities(self, text: str, cities_list: List[str], debug: bool = False) -> str:
        """Удаляет названия городов из текста, используя один скомпилированный regex"""
        if not cities_list or not text:
            return text
        
        # Быстрая проверка: если текст короткий, города вряд ли есть
        if len(text) < 15:
            return text
        
        cache_key = self._get_cache_key(cities_list)
        
        # Получаем или создаём скомпилированный regex
        if cache_key not in self._city_regex_cache:
            # Сортируем по длине (сначала длинные названия)
            sorted_cities = sorted(cities_list, key=len, reverse=True)
            escaped_cities = [re.escape(city) for city in sorted_cities]
            # Создаём один regex для всех городов
            pattern = r'\b(?:' + '|'.join(escaped_cities) + r')\b'
            self._city_regex_cache[cache_key] = re.compile(pattern, re.IGNORECASE)
            if debug:
                print(f"  📝 Создан regex для {len(cities_list)} городов")
        
        # Одна операция вместо тысячи
        result = self._city_regex_cache[cache_key].sub(' ', text)
        
        # Кэшируем результат для этого текста (LRU)
        result_cache_key = (text, cache_key)
        self._remove_cities_cache[result_cache_key] = result
        if len(self._remove_cities_cache) > self._cache_maxsize:
            self._remove_cities_cache.popitem(last=False)
        
        return self._compiled['spaces'].sub(' ', result).strip()
    
    
    def _detect_country_fast(self, text: str) -> str:
        """
            Быстрое определение страны без цикла
        """
        text_lower = text.lower()
        if 'россии' in text_lower or 'россия' in text_lower:
            return 'Россия'
        if 'испании' in text_lower or 'испания' in text_lower:
            return 'Испания'
        if 'италии' in text_lower or 'италия' in text_lower:
            return 'Италия'
        if 'германии' in text_lower or 'германия' in text_lower:
            return 'Германия'
        return None
    ...
    def preparatory_cleaner(self, text: str, debug: bool = False) -> str:
        """
            Общий очиститель для подготовки для дальнейшего очищения
        """
        if not text:
        ...
        # Проверяем кэш
        if text in self._prep_cache:
            if debug:
                print(f"  🔄 Нашёл в кэше: '{text[:50]}...'")
            return self._prep_cache[text]
        ...
        # Проверка на сборник мультфильмов
        mf_count = result.count('м/ф') + result.count('m/ф')
        if mf_count >= 2 and ('мультфильм' in result or 'мультсериал' in result):
            self._prep_cache[text] = 'серия мультфильмов'
            return 'серия мультфильмов'
        
        # Удаление подстрок типа '№5'
        result = self._compiled['hash_numbers'].sub(' ', result)
        
        # Удаление возрастных ограничений
        result = self._compiled['age'].sub(' ', result)
        
        # Удаление тире между цифрами
        result = re.sub(r'(\d+)[-—–]+|[-—–]+(\d+)', lambda m: m.group(1) or m.group(2), result)
        
        # Удаление пунктуации
        result = self._compiled['punctuation'].sub(' ', result)
        
        # Удаление номеров частей, серий, сезонов, фильмов
        result = self._compiled['parts'].sub(' ', result)
        result = self._compiled['series'].sub(' ', result)
        result = self._compiled['seasons'].sub(' ', result)
        result = self._compiled['films_num'].sub(' ', result)
        
        # Очистка пробелов
        result = self._compiled['spaces'].sub(' ', result).strip()
        
        # Сохраняем в кэш
        self._prep_cache[text] = result
        
        return result
    
    
    def final_clean(self, text: str, debug: bool = False) -> str:
        """
            Финальная очистка, которая одинакова для всех категорий
        """
        if not text:
        ...
        result = self._compiled['four_digits'].sub(' ', result)
        
        # Удаление паттерна "цифры + дефис + буквы"
        result = self._compiled['digit_letters'].sub(' ', result)
        
        # Финальное форматирование пробелов
        result = self._compiled['spaces'].sub(' ', result).strip()
    ...
    def clean_not_sport(self, text: str, debug: bool = False) -> str:
        """
            Метод для очистки неспортивных телепрограмм
        """
        if not text:
        ...
        result = self.preparatory_cleaner(text, debug)
        
        # Удаление ненужной информации
        result = self._compiled['not_major'].sub(' ', result)
        result = self._compiled['spaces'].sub(' ', result).strip()
        ...
        for special_name in self.BASE_SPECIAL_NAMES:
            if result_lower == special_name.lower().strip():
        ...
        for special_pattern in self.SPECIAL_PATTERNS_FOR_SAVE:
            match = re.search(special_pattern, result, flags = re.IGNORECASE)
            if match:
                return self._compiled['spaces'].sub(' ', match.group(0)).strip()
        
        # Проверка на ключевые имена
        for program in self.PROGRAMS_TO_REMAIN_CATEGORIES['not_sport']:
            if program in result_lower:
                return program.strip()
        
        # Финальная очистка
        return self.final_clean(result, debug)
    
    def clean_films(self, text: str, debug: bool = False) -> str:
        """Метод для очистки фильмов, документальных сериалов и тд."""
        if not text:
            return ''
        ...
        # Проверка на ключевые названия
        for program in self.PROGRAMS_TO_REMAIN_CATEGORIES['films']:
            if program in result_lower:
                return program.strip()
        
        # Удаление стоп-паттернов
        if 'films' in self._sorted_stop_patterns:
            for pattern in self._sorted_stop_patterns['films']:
                result = re.sub(pattern, ' ', result, flags=re.IGNORECASE)
        
        # Удаление стоп-слов
        if 'films' in self._sorted_stop_words:
            for word in self._sorted_stop_words['films']:
                result = re.sub(r'\s*' + word + r'\s*', ' ', result, flags=re.IGNORECASE)
        
        result = self._compiled['spaces'].sub(' ', result).strip()
        
        return self.final_clean(result, debug)
    
    def clean_sport_programs(self, text: str, cities_dict: Dict[str, List[str]], debug: bool = False) -> str:
        """Метод для очистки спортивных событий с оптимизированным удалением городов"""
        if not text:
            return ''
        
        result = self.preparatory_cleaner(text, debug)
        
        # Удаление градаций финала
        result = self._compiled['final_stage'].sub(' ', result)
        result = self._compiled['final_variants'].sub(' ', result)
        
        # Удаление номеров матчей и попыток
        result = self._compiled['match_number'].sub(' ', result)
        result = self._compiled['attempt_number'].sub(' ', result)
        
        # Удаление спонсоров (используем скомпилированный паттерн)
        result = self._compiled['sponsors'].sub(' ', result)
        
        # Замена некоторых названий
        if 'чемпионат россии премьер лига' in result:
            result = self._replace_whole_word(result, 'чемпионат россии премьер лига', 'рпл')
        elif 'футзал' in result and 'суперлига' in result:
            result = self._replace_whole_word(result, 'суперлига', 'чемпионат россии')
        elif 'футбол' in result:
            if 'первая лига' in result:
                result = self._replace_whole_word(result, 'первая лига', 'чемпионат россии первая лига')
        ...
        # Проверка на ключевые названия
        result_lower = result.lower()
        for program in self.PROGRAMS_TO_REMAIN_CATEGORIES['sport']:
            if program in result_lower:
                return program.strip()
        
        # Удаление стоп-паттернов
        if 'sport' in self._sorted_stop_patterns:
            for pattern in self._sorted_stop_patterns['sport']:
                result = re.sub(r'\s*' + pattern + r'\s*', ' ', result, flags=re.IGNORECASE)
        
        # Удаление стоп-слов
        if 'sport' in self._sorted_stop_words:
            for word in self._sorted_stop_words['sport']:
                result = re.sub(r'\s*' + word + r'\s*', ' ', result, flags=re.IGNORECASE)
        
        result = self._compiled['spaces'].sub(' ', result).strip()
        
        # УДАЛЕНИЕ ГОРОДОВ (ОПТИМИЗИРОВАННО)
        # Быстро определяем страну
        country = self._detect_country_fast(result)
        
        if country and country in cities_dict:
            # Проверяем, есть ли вообще города в тексте (быстрая проверка)
            if len(result) > 15:  # Только для достаточно длинных текстов
                # Берём первые 10 городов для быстрой проверки
                sample_cities = cities_dict[country][:10]
                if any(city.lower() in result_lower for city in sample_cities):
                    result = self.remove_cities(result, cities_dict[country], debug)

        elif len(result) > 15:
            # Если страна не определена, пробуем Россию (наиболее частый случай)
            if 'Россия' in cities_dict:
                sample_cities = cities_dict['Россия'][:10]
                if any(city.lower() in result_lower for city in sample_cities):
                    result = self.remove_cities(result, cities_dict['Россия'], debug)
        
        # Удаление паттерна "буква+цифра"
        result = self._compiled['letter_digit'].sub(' ', result)
        result = self._compiled['spaces'].sub(' ', result).strip()
        
        return self.final_clean(result, debug)
    
    
    def process_program_by_type(
            self, 
            text: str, 
            category_type: str, 
            cities_dict: dict, 
        ...
        """
        if category_type not in ['sport', 'not_sport', 'films', 'other']:
            raise ValueError(f"Недопустимое значение category_type: {category_type}")
        
        if category_type == 'sport':
            return self.clean_sport_programs(text, cities_dict, debug)
        elif category_type == 'not_sport':
            return self.clean_not_sport(text, debug)
        elif category_type == 'films':
            return self.clean_films(text, debug)
        else:  # other
            return self.clean_not_sport(text, debug)
    
    def divide_by_categories(self, data: pd.DataFrame, table_form: str) -> Dict[str, List[str]]:
        """Разделение программ по категориям"""
        if table_form == 'vimb':
            categories = {
                'films': ['анимационный', 'художественный фильм', 'документальный', 'худ. фильм', 'мультфильм'],
                'not_sport': ['лица страны', 'матч! парад', 'все на матч', 'всенаматч', 'новости', 'век нашего спорта', 
                ...
                'sport': ['автоспорт', 'аквабайк', 'акробатический рок-н-ролл', 'американский футбол', 
                        ...
                        'настольный футбол', 'ориентирование', 'олимпийские игры', 'падел', 'плавание', 'панкратион',
                        'прыжки в воду', 'прыжки на лыжах', 'прыжки с трамплина на лыжах', 'пауэрлифтинг', 
                        'прыжки на батуте и акробатической дорожке', 'парашютный спорт', 'паркур', 'пейнтбол', 
                        'пятиборье', 'регби', 'рпл', 'рукопашный бой', 'роллер спорт', 'рафтинг', 'реслинг', 
                        'самбо', 'санный спорт', 'скелетон', 'скоростной спуск на коньках', 'смешанные единоборства', 
                        'спортивная гимнастика', 'стрельба из лука', 'серфинг', 'стрельба', 'спортивные танцы', 
                        'сноуборд', 'скалолазание', 'сквош', 'софтбол', 'спортивная аэробика', 'теннис', 'триатлон', 
                        'тяжёлая атлетика', 'тяжелая атлетика', 'тхэквондо', 'тайский бокс', 'танцевальный спорт',  
                        ...
                        'фестиваль экстремальных видов спорта', 'художественная гимнастика', 'хайдайвинг', 
                        'хоккей на траве', 'чемпионат испании', 'чемпионат по робототехнике', 'чемпионат италии', 
            ...
            for cat in ['films', 'not_sport', 'sport']:
                mask = remaining['Название программы'].str.contains(patterns[cat], case = False, na = False)
                result[cat] = remaining[mask]['Название программы'].tolist()
                remaining = remaining[~mask]
            result['other'] = remaining['Название программы'].tolist()
            ...
            raise ValueError(f"Неизвестный вид таблицы! Выберите либо 'vimb', либо 'palomars'.")
    
    def clean_dataframe(self, df: pd.DataFrame, table_form: str, cities_dict: Dict[str, List[str]], 
                        program_name_column: str = 'Название программы') -> Tuple[List[str], pd.DataFrame]:
        """Очистка всего DataFrame с кэшированием результатов"""
        # Разбивка программ по категориям
        programs_dict = self.divide_by_categories(df, table_form)
        
        # Кэш для результатов очистки
        if not hasattr(self, '_clean_cache'):
            self._clean_cache = {}
        
        # Собираем все уникальные программы
        all_programs = set()
        for programs in programs_dict.values():
            all_programs.update(programs)
        
        # Обрабатываем каждую уникальную программу один раз
        unified_mapping = {}
        for program in all_programs:
            # Определяем категорию программы
            category = None
            for cat, progs in programs_dict.items():
                if program in progs:
                    category = cat
                    break
        ...
        # Возвращаем уникальные очищенные программы
        unique_programs = list(set(unified_mapping.values()))
...
class MusicChannelCleaner:
    ...
    def __init__(self, channel):
        ...
        self.BASE_SPECIAL_NAMES = [
                ...
                'анимационный сериал фильм', 'анимационный фильм сериал', 'документальный цикл',
                'документальный цикл', 'мультфильм', 'мультфильмы', 'юмористический концерт', 
                'короткометражные х фильмы', 'мультсериал', 'серия мультфильмов', 'концерт',
                'сериал', 'худ фильм', 'худ фильм сериал', 'док фильм сериал', 'док сериал фильм', 
                'сериал фильм', 'сериал х ф', 'х фильмы', 'док фильм'
        ]
